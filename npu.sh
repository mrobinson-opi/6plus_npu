...
# Based on CIX Linux System User Manual
...
echo "Note: This repository is large (approx 60GB-120GB). Ensure you have space."
read -p "Do you want to download the Model Hub now? (y/n): " download_choice

if [[ "$download_choice" == "y" ]]; then
    echo "Installing Git LFS..."
    # Corrected: Ensures clean package name
    apt-get install -y git-lfs
    
    echo "Cloning repository (this will download ~60GB of models)..."
    # Clone to /home/orangepi/
    cd /home/orangepi
    ...
    if [ -d "$HUB_DIR" ]; then
        ...
        echo -e "${RED}Download failed or directory creation issue.${NC}"
        exit 1
    fi
else
...
if [[ "$dep_choice" == "y" ]]; then
    ...
    # Corrected: Ensures clean package names
    apt-get install -y cmake python3-pip

    if [ -d "$HUB_DIR" ]; then
        echo "Installing Python requirements from Hub..."
        # Navigate to the correct directory and install requirements
        cd "$HUB_DIR"
        # Using --break-system-packages to work around potential system Python conflicts
        pip3 install -r requirements.txt --break-system-packages
        echo -e "${GREEN}System and Python dependencies installed.${NC}"
    else
        echo -e "${RED}Model Hub directory not found at $HUB_DIR. Cannot install specific Python requirements.${NC}"
    fi
fi
...
if [[ "$runtime_choice" == "y" ]]; then
    ...
    # Find the pre-built wheel file provided by the system image
...
if [[ -n "$DRIVER_CHECK" ]]; then
    ...
    echo "$DRIVER_CHECK"
    echo "Manual recommends version 1.1.0 or higher."
else
...
echo -e "\n${GREEN}================================================${NC}"
echo -e "${GREEN}   Setup script complete. Reboot may be required.   ${NC}"
echo -e "${GREEN}================================================${NC}"
